...
| 端口监听 | 🟢 正常 | 0.0.0.0:33000 |
| 服务器本地访问 | 🟢 成功 | curl 127.0.0.1:33000 返回 200 OK |
| 后端API | 🟢 运行中 | 0.0.0.0:38000 |
...
| 33000 | 0.0.0.0:33000 | next-server | ❌ 被阻止 |
| 38000 | 0.0.0.0:38000 | uvicorn | ❌ 被阻止 |
...
3. 添加入站规则：
   - 端口 33000，协议 TCP，来源 0.0.0.0/0
   - 端口 38000，协议 TCP，来源 0.0.0.0/0
4. 保存规则

**然后访问**：
- 前端：http://144.202.3.175:33000
- 后端：http://144.202.3.175:38000
...
```bash
# 在本地电脑运行
ssh -L 33000:127.0.0.1:33000 -L 38000:127.0.0.1:38000 kcriss@144.202.3.175
```
...
### 端口监听状态
```
LISTEN 0.0.0.0:33000    (next-server, PID 1384733)
LISTEN 0.0.0.0:38000    (uvicorn, PID 1330470)
LISTEN 0.0.0.0:40003   (uvicorn, PID 572481) ✅ 公网可访问
```

### 本地访问测试
```bash
$ curl http://127.0.0.1:33000
HTTP/1.1 200 OK ✅
...
2. 找到该服务器的安全组设置
3. 添加入站规则：
   - TCP 33000 (来源: 0.0.0.0/0)
   - TCP 38000 (来源: 0.0.0.0/0)
4. 保存并应用

**完成后即可访问**：
```
http://144.202.3.175:33000
```
