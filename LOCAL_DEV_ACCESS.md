...
**工作原理**：
```
您的浏览器 → localhost:3000 (本地)
              ↓ [SSH隧道]
服务器 → localhost:40004 (服务器上的前端)
       → localhost:8000 (服务器上的后端)
```
...
然后访问：
- 直接访问: `http://10.212.227.125:3000`
- SSH转发: `ssh -L 3000:localhost:3000 -L 8000:localhost:8000 kcriss@10.212.227.125`
...
### Q: SSH端口转发连接后，localhost:3000 还是无法访问？
